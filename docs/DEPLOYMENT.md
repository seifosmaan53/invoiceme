...
Automated backups via `scripts/backup-database.sh` and `scripts/setup-cron-backup.sh`; restore via `scripts/restore-database.sh`.

---

## Kubernetes

Manifests in `/k8s` split by resource:

- `k8s/deployment.yaml` — Deployment (3 replicas, liveness/readiness probes on `/api/health`)
- `k8s/service.yaml` — LoadBalancer Service exposing port 80 → container port 3000
- `k8s/hpa.yaml` — HorizontalPodAutoscaler (2–10 replicas, scales on CPU/memory utilization)

```bash
kubectl apply -f k8s/
```

The backend is stateless (sessions live in Redis/Postgres), so horizontal scaling requires no extra coordination beyond the Service and HPA above. Secrets (`DB_HOST`, `DB_PASSWORD`, etc.) are expected in a `invoiceme-secrets` Kubernetes Secret — create it before applying the Deployment.
